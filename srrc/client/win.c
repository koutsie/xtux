...
void win_update(void)
{
    ...
    if( client.debug )
        printf(INFO "Redrawing status bar!\n");
...
void win_set_properties(char *title, int new_w, int new_h)
{
    ...
    if( client.debug )
    printf(INFO "Setting window properties to %d,%d...", win.width, win.height);
...
void win_grab_pointer(void)
{
    ...
    printf(ERR "Error grabbing Cursor! val = %d\n", status);
...
void wait_till_expose(int timeout)
{
    ...
    if( client.debug )
    printf(INFO "wait_till_expose: Timed out!\n");
