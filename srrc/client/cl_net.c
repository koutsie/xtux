...
int cl_network_connect(char *sv_name, int port)
{
    ...
    switch( client.demo ) {
    case DEMO_RECORD:
    printf(INFO "RECORDING Connection.......\n");
    ...
    if( server == NULL ) {
    printf(ERR "Couldn't create new connection\n");
    close(sock);
    sock = -1;
    return 0;
    }
    ...
    printf(CLIENT "Joining Game....\n");
    cl_netmsg_send_join();
    printf(CLIENT "Querying Server info......\n");
    ...
    while( (msg = net_next_message(server, NM_ALL)).type != NETMSG_NONE ) {
        ...
        printf(INFO "IGNORING message type %d!\n", msg.type);
        }
    }
    }

    alarm(0);
    printf(ERR "TIMED OUT waiting for server info!\n");
...
{
    printf(ERR "NOT HANDLING MESSAGE \"%s\"\n", net_message_name(msg.type));
}
...
int cl_net_handle_message(netmsg msg)
{
    ...
    if( client.debug )
    printf(INFO "cl_net_handle_message: type = %s\n",
           net_message_name(msg.type));
    ...
    printf(ERR "cl_netmsg_recv: Unknown message type %d!\n", msg.type);
    return -1;
    }
...
int cl_net_update(void)
{
    ...
    if( client.debug ) {
    printf(INFO "**************************\n");
    printf(INFO "cl_net_update(): About to get read() from server.....\n");
    }
    ...
    if( client.demo == DEMO_PLAY ) { /* END of the demo. */
        ...
        printf(INFO "cl_net_update(): net_input_flush() returned %d\n", status);
        cl_net_connection_lost(NULL);
    }
    return status;
    } else if( status == 0 ) {
    printf(INFO "Skipping this loop, but trying again\n");
    return status;
    }
