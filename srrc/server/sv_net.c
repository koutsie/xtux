...
void sv_net_get_client_input(void)
{
    ...
    if( nc->incoming ) {
        ...
        if( ++cl->bad >= MAX_BAD ) {
            ...
            printf(ERR "Read error %d for %s\n", cl->bad, cl->name);
        } else {
        printf(ERR "Input error from \"%s\" (%s).\n",
...
static void sv_net_add_client(int fd)
{
    ...
    printf(ERR "Error creating new net connection.\n");
    return;
    }
    ...
    printf(CLIENT "Client joining from \"%s\"\n", nc->remote_address);
...
static int sv_netmsg_not_handled(client_t *cl, netmsg msg)
{
    ...
    printf(ERR "NOT HANDLING MESSAGE \"%s\" from \"%s\" (%s)\n",
...
static int sv_net_handle_message(client_t *cl, netmsg msg)
{
    ...
    printf(ERR "sv_netmsg_recv: Unknown message type %d from %s!\n",
           msg.type, cl->name);
    return -1;
    }
