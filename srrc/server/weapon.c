...
void weapon_init(void)
{
    ...
    if( et->class == ITEM && et->item_action == GIVEWEAPON ) {
        ...
        printf(ERR "Couldn't match weapon type for item %s\n", et->name);
        et->item_type = 0;
        }
        ...
        printf(ERR "Couldn't match weapon type for entity %s\n", et->name);
        et->weapon = 0;
        }
    }
...
void weapon_update(void)
{
    ...
    if( ent->weapon_switch  && !( ent->powerup & ( 1<<PU_FIXED_WEAPON ) ) ) {
        ...
        do {
            ...
            printf(ERR "entity has no weapons!\n");
            weap = WT_NONE;
            break;
            }
...
void weapon_fire(entity *ent)
{
    ...
    printf(ERR "Error firing weapon %d for entity %d (%s)\n",
