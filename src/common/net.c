...
static void net_mark_full_frame(netbuf_t *in, nm_frame_t nm)
{
	...
	for (p = in->pos; p < in->size; p += size)
	{
		...
		{
			printf(ERR "ERROR: Type (%d) out of range!\n", type);
			printf(ERR "in->pos = %d, in->size = %d, p = %d\n",
				   in->pos, in->size, p);
			in->size = 0;
			return;
		}
...
void net_put_message(netconnection_t *nc, netmsg msg)
{
...
#if NET_DEBUG
	printf(INFO "net_put_message(%s)\n", net_message_name(msg.type));
#endif

	type = (int) msg.type;
	if (type >= NUM_NETMESSAGES)
	{
		printf(ERR "Unknown message type %d\n", type);
		return;
	}
	...
	{
		printf(INFO "Output full. Auto-flushing.\n");
		net_output_flush(nc);

		if (nc->output.size + size > NETBUFSIZ)
		{
			printf(ERR "Could not write output to %s. Discarding buffer\n",
				   nc->remote_address);
			nc->output.size = 0;
			return;
		}
...
netmsg net_next_message(netconnection_t *nc, nm_frame_t last_frame)
{
	...
	{
		printf(INFO "Waiting for the rest of frame......\n");
		return msg;
	}

	if (in->end - in->start <= 0)
	{ /* Nothing in input buffer */
		printf(INFO "in->start = %d, in->end = %d!\n", in->start, in->end);
		return msg; /* NONE */
	}
	...
	for (p = in->pos; p < in->end; p += size)
	{
		...
		if (type >= NUM_NETMESSAGES)
		{
			printf(ERR "Net: Unknown message type from client.\n");
			return msg;
		}
