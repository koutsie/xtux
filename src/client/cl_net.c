...
int cl_network_connect(char *sv_name, int port)
{
	...
	{ /* A real connection, not just a demo */
...
int cl_net_handle_message(netmsg msg)
{
	...
	{
		printf(ERR
		"cl_netmsg_recv: Unknown message type %d!\n", msg.type);
		return -1;
	}
...
int cl_net_update(void)
{
	...
	if (client.debug)
	{
		...
		"**************************\n");
		printf(INFO
		"cl_net_update(): About to get read() from server.....\n");
	}
	...
	{ /* Check for errors */
		...
		{
			printf(INFO
			"cl_net_update(): net_input_flush() returned %d\n", status);
	...
	{
		printf(INFO
		"Skipping this loop, but trying again\n");
		return status;
	}
