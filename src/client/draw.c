           ...
           int width, int height)
{
    ...
    printf(ERR "%s: ** ERROR LOADING CLIPMAP %s! **\n", __FILE__, client.map);
    client.state = MENU;
    return;
    } else {
...
int tileset_load(char *tname)
{
    ...
    printf(INFO "%s: Using default tileset.\n", __FILE__);
    ...
    if( strncasecmp("BASE", buf, 4) == 0 ) {
        if( client.debug )
        printf(INFO "%s: Reading Base tileset table\n", __FILE__);
        layer = BASE;
        masking = NOMASK;
    } else if( strncasecmp("OBJECT", buf, 6) == 0 ) {
        if( client.debug )
        printf(INFO "%s: Reading Object tileset table\n", __FILE__);
        layer = OBJECT;
        masking = MASKED;
...
void redraw_map_buf(int x_off, int y_off)
{
    ...
    for( x=x_off/TILE_W; x < x_max ; x++) {
        ...
        printf(ERR "No tile in base[%d] = %c\n", i, map->base[i]);
...
void draw_map_toplevel(shortpoint_t screenpos)
{
    ...
    printf(ERR "draw_map_toplevel() called when no toplevel exists!\n");
    }
    ...
    for( x = x_off ; x < x_max ; x++ ) {
        ...
        printf(INFO "draw_map_toplevel(): i (%d) >= (%d) = map wdth*hght\n",
        ...
        if( (tile = ts.o_table[(int)map->toplevel[i]][0]) == -1){
            printf(ERR "No tile in toplevel[%d][0] = %c\n",
               i, map->toplevel[i]);
            tile = image("bad_tile.xpm", MASKED)->pixmap;
        }

        if((mask = ts.o_table[(int)map->toplevel[i]][1]) == -1){
            printf(ERR "No mask in toplevel[%d][1] = %c\n",
               i, map->toplevel[i]);
            mask = image("bad_tile.xpm", MASKED)->mask;
        }
