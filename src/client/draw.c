...
int tileset_load(char *tname)
{
	...
	for (l = 1; !feof(file); l++)
	{
		...
		} else if (strncasecmp("OBJECT", buf, 6) == 0)
		{
			if (client.debug)
				printf(INFO
			"%s: Reading Object tileset table\n", __FILE__);
			layer = OBJECT;
			masking = MASKED;
...
void draw_map_toplevel(shortpoint_t screenpos)
{
	...
	{
		printf(ERR
		"draw_map_toplevel() called when no toplevel exists!\n");
	}
	...
	for (y = y_off; y < y_max; y++)
	{
		for (x = x_off; x < x_max; x++)
		{
			...
			{
				printf(INFO
				"draw_map_toplevel(): i (%d) >= (%d) = map wdth*hght\n",
