...
int event_setup_finish(void)
{
    ...
    if( ptr->event.good ) {
        ...
        printf(INFO "Dropping event type %s\n", event_name[ptr->event.type]);
    ...
    if( event_tab[i].type == EVENT_TRIGGER ) {
        ...
        printf(ERR "No such event \"%c\" for trigger\n",
...
static void event_new_teleport(event_t *event, const char *args)
{
    ...
    printf(ERR "Error parsing teleport event, args = \"%s\"\n", args);
    }
...
static void event_new_damage(event_t *event, const char *args)
{
    ...
    printf(ERR "Error parsing damage object\n");
    }
...
static void event_new_map_target(event_t *event, const char *args)
{
    ...
    printf(ERR "Error parsing map target event\n");
    return;
    }
...
static void event_new_trigger(event_t *event, const char *args)
{
    ...
    printf(ERR "No letter, ON/OFF flag or val in trigger \"%s\"\n",args);
    return;
    }
...
static void event_new_lookat(event_t *event, const char *args)
{
    ...
    if( sscanf(args, "%d", &dir) != 1 ) {
        printf(ERR "No lookat dir found.\n");
        return;
    }
...
int event_get_virtual_entity(const char *args, int *len)
{
    ...
    printf(ERR "Error: '>' not found searching for virtual entity\n");
    return -1;
    }
    ...
    *len = p + 1 - args;
    ...
    printf(ERR "No entity type found in \"%s\"\n", args);
    return -1;
...
void read_event(const char *line)
{
    ...
    if( !strncasecmp(line, event_name[i], len) ) {
        ...
        printf(ERR "Error reading EVENT symbol \"%s\"\n", line);
        break;
        }
        ...
        printf(ERR "Error reading 4 EVENT flags from \"%s\"\n", line);
        break;
        }
        ...
        if( *args == '>' )
            ...
            printf(ERR "ERROR reading vk_cmp in line = \"%s\"\n", line);
            break;
        }

        args++;
        if( sscanf(args, "%d", &key) != 1 ) {
            printf(ERR "ERROR reading key value in line = \"%s\"\n", line);
            break;
        }
