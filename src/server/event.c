...
int event_setup_finish(void)
{
	...
	for (i = 0; i < events; i++)
	{
		if (event_tab[i].type == EVENT_TRIGGER)
		{
			...
			if (ep)
			{
				if (event_tab[i].data.trigger.on)
				{ /* Do initial trigger */
					event_t *targ;
	...
	for (y = y_start; y < y_max; y++)
	{
		for (x = x_start; x < x_max; x++)
		{
			if ((e = map->event[y * map->width + x]))
			{
				...
				{  /* If no virtual entity maybe some pixel effects */
...
static void event_new_teleport(event_t *event, const char *args)
{
	...
	{
		printf(ERR
		"Error parsing teleport event, args = \"%s\"\n", args);
	}
...
static void event_new_map_target(event_t *event, const char *args)
{
	...
	{
		printf(ERR
		"Error parsing map target event\n");
		return;
	}
...
static void event_new_trigger(event_t *event, const char *args)
{
	...
	{
		printf(ERR
		"No letter, ON/OFF flag or val in trigger \"%s\"\n", args);
		return;
	}
...
int event_get_virtual_entity(const char *args, int *len)
{
	...
	{
		printf(ERR
		"Error: '>' not found searching for virtual entity\n");
		return -1;
	}
...
void read_event(const char *line)
{
	...
	for (i = 0; i < NUM_EVENTS; i++)
	{
		...
		if (!strncasecmp(line, event_name[i], len))
		{
			...
			{
				printf(ERR
				"Error reading 4 EVENT flags from \"%s\"\n", line);
				break;
			}
			...
			if (!strncmp(args, "KEY", 3))
			{
				...
				{
					printf(ERR
					"ERROR reading vk_cmp in line = \"%s\"\n", line);
					break;
				}
				...
				{
					printf(ERR
					"ERROR reading key value in line = \"%s\"\n", line);
					break;
				}
