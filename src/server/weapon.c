...
void weapon_init(void)
{
	...
	for (i = 0; i < num_entity_types; i++)
	{
		...
		if (et->class == ITEM && et->item_action == GIVEWEAPON)
		{
			...
			{
				printf(ERR
				"Couldn't match weapon type for item %s\n", et->name);
				et->item_type = 0;
			}
		} else if (et->weapon_name)
		{
			...
			{
				printf(ERR
				"Couldn't match weapon type for entity %s\n", et->name);
				et->weapon = 0;
			}
		}
...
void weapon_fire(entity *ent)
{
	...
	{
		printf(ERR
		"Error firing weapon %d for entity %d (%s)\n",
...
void weapon_hit(entity *shooter, entity *victim, point P, byte weaptype)
{
	...
	damage = wt->damage;
	if (wt->class == WT_BEAM)
	{ /* Damage is done over a second for beams */
		damage /= server.fps;
		if (damage < 1)
		{
