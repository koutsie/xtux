...
int sv_netmsg_recv_join(client_t *cl, netmsg msg)
{
    ...
    {
        printf(ERR
        "Client attempted join with wrong protocol\n");
    ...
    if (change)
        ...
        {
            ERR_QUIT(ERR
            "Error changing levels", EXIT_FAILURE);
        }
...
int sv_netmsg_recv_ready(client_t *cl, netmsg msg)
{
    ...
    {
        printf(ERR
        "ERROR! Client has not yet sent join!\n");
