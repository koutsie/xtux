...
{
    printf(CLIENT "Got no-op from %s\n", cl->name);
    return 0;
}
...
{
    printf(CLIENT "Remote version for %s = %c.%c.%c\n", cl->name,
...
int sv_netmsg_recv_join(client_t *cl, netmsg msg)
{
    ...
    printf(ERR "Joining client name eq SERVER!\n");
    ...
    if( sv_map_changelevel(msg.join.map_name, msg.join.gamemode) < 0 ) {
        ERR_QUIT(ERR "Error changing levels", EXIT_FAILURE);
    }
...
int sv_netmsg_recv_ready(client_t *cl, netmsg msg)
{
    ...
    printf(INFO "GOT READY\n");

    if( cl->status < JOINING ) {
    printf(ERR "ERROR! Client has not yet sent join!\n");
...
int sv_netmsg_recv_away(client_t *cl, netmsg msg)
{
    ...
    printf(CLIENT "%s sent away message\n", cl->name);
