...
static void sv_net_update(void)
{
	...
	if (FD_ISSET(sock, &read_fds))
	{
		...
		} else
		{
			...
			{ /* Too many clients, send rejection */
...
static void sv_net_add_client(int fd)
{
	...
	{
		printf(ERR
		"Error creating new net connection.\n");
		return;
	}
...
static int sv_netmsg_not_handled(client_t *cl, netmsg msg)
{
	...
	printf(ERR
	"NOT HANDLING MESSAGE \"%s\" from \"%s\" (%s)\n",
...
static int sv_net_handle_message(client_t *cl, netmsg msg)
{
	...
	{
		printf(ERR
		"sv_netmsg_recv: Unknown message type %d from %s!\n",
				msg.type, cl->name);
		return -1;
	}
